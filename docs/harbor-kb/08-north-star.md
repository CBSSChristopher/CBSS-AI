...
Harbor (ElevenLabs agent + Yard CRM + Cursor box `bc-711f8685-d818-473d-b1ac-1fd96e69e69c`) is the **unified CBSS desk**. **Workflow first:** get_next_lead pulls due follow-ups on Harbor-assigned leads first, then New/Unassigned → if they share a use, genuine “yeah I love that use” rapport → qualify → ZIP quote → CRM note/disposition/CTE → ready-to-buy notifies Christopher Banks (default) + Bryan Reese. New/Unassigned is global. Due follow-ups are Harbor-owner only. **“New” = one-trip / like-new** (not factory brand-new). Used stays used. Cards frozen. Call + email only. **Twilio import last.** Dialing gated on **arm**.
