...
- Assigned rep for **CTE** emails comes from the contact owner matched to an active Yard login (`cycle:users`). Missing / inactive rep or missing client email pauses CTE and flags — never invents an email for an unknown name. **Harbor** is a valid assigned rep for that same `fireTemplate` path (`harbor@cbshippingsolutions.com`, Reply-To Harbor) even without a human Yard login. No-answer, voicemail, and soft-delay on a Harbor card send the current CTE template live. Dial stays parked unless `VA_DIAL_ARMED`.
