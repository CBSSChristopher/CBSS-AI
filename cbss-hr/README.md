# CBSS Employee Benefits Package

Universal team overview for CBGC LLC DBA CB Shipping Solutions.

- Source: `cbss-employee-benefits.html`
- PDF: `CBSS-Employee-Benefits-Package.pdf`

Rebuild the PDF:

```bash
google-chrome --headless --disable-gpu --no-sandbox --no-pdf-header-footer \
  --print-to-pdf=cbss-hr/CBSS-Employee-Benefits-Package.pdf \
  file://$PWD/cbss-hr/cbss-employee-benefits.html
```

This is a general overview. Official plan documents and partner agreements govern.
