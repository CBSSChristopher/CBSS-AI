...
function applyCompetitorPick(pull, pick) {
  return {
    ...pull,
    lines: pull.lines.filter((line) => line.size === pick.size && line.grade === pick.grade && line.config === pick.config),
  };
}
function formatCompetitorCard(pull) {
  const rows = pull.lines.map((line) => {
    const miles = line.miles == null ? "" : ` · ${line.miles} mi`;
    return `${line.size} ${line.grade} · ${line.config}  $${Math.round(line.delivered).toLocaleString("en-US")} delivered  ${line.depot}${miles}`;
  });
  return [
