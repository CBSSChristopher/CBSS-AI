...
import { contactStage, findContact } from "./crm";
...
export function followUpFromKind(kind: InboundKind, now = new Date()): { nextAction: string; followUpDate: string; stage: string } {
  ...
  if (kind === "shopping") {
    return { nextAction: "Check back — they have the proposal and are comparing quotes.", followUpDate: day(3, 9), stage: "Proposal Sent" };
  }
  if (kind === "ready") {
    return { nextAction: "They are ready. Confirm site and collect before the truck.", followUpDate: day(0, 10), stage: "Proposal Sent" };
  }
  if (kind === "question" || kind === "need_info") {
    return { nextAction: "Reply to their question. Get ZIP / site if still NEED.", followUpDate: day(1, 9), stage: "Quote" };
  }
  if (kind === "quiet") {
...
export function mailNoteText(log: MailLog): string {
...
export function outboundFollowup(hasProposal: boolean, now = new Date()): { nextAction: string; followUpDate: string } {
  const d = new Date(now.getTime() + (hasProposal ? 2 : 1) * 24 * 60 * 60 * 1000);
  const iso = new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
  const followUpDate = iso.slice(0, 11) + "09:00";
  return {
...
export function upsertDealStage(deals: CrmDeal[], contact: CrmContact, stage: string, amount?: string): CrmDeal[] {
