...
test("tasks tab exposes a Complete button and completeTask handler", () => {
  ...
  assert.match(html, /data-owner="New\/Unassigned"/);
  assert.match(html, /New\/Unassigned/);
...
test("worker still serves the CRM data routes used by the desk", async () => {
  ...
  assert.match(worker, /crmBuild: 23/);
  assert.match(worker, /x-crm-build", "23"/);
  assert.match(worker, /UNASSIGNED_POOL/);
  assert.match(worker, /action === "ingestWebsiteLead"/);
  assert.match(worker, /writeCrmSnapshot/);
  assert.match(wrangler, /15 6 \* \* \*/);
